...
class HDC_module(nn.Module):
    def __init__(self, in_dim, out_dim, activation):
        ...
        self.conv_331_1 = nn.Conv3d(self.out_inter_dim, self.out_inter_dim, kernel_size=(3, 3, 1), stride=1,
                                  padding=(1, 1, 0), bias=False)
        self.bn_layer_1 = SynchronizedBatchNorm3d(self.out_inter_dim)

        self.conv_331_2 = nn.Conv3d(self.out_inter_dim, self.out_inter_dim, kernel_size=(3, 3, 1), stride=1,
                                  padding=(1, 1, 0), bias=False)
        self.bn_layer_2 = SynchronizedBatchNorm3d(self.out_inter_dim)

        self.conv_331_3 = nn.Conv3d(self.out_inter_dim, self.out_inter_dim, kernel_size=(3, 3, 1), stride=1,
                                  padding=(1, 1, 0), bias=False)
        self.bn_layer_3 = SynchronizedBatchNorm3d(self.out_inter_dim)

        self.pool = max_pooling_3d_noz()
        self.conv_331_4 = nn.Conv3d(self.out_inter_dim, self.out_inter_dim, kernel_size=(3, 3, 1), stride=1,
                                  padding=(1, 1, 0), bias=False)
        self.bn_layer_4 = SynchronizedBatchNorm3d(self.out_inter_dim)
        ...
        self.conv_133 = nn.Conv3d(self.out_inter_dim, self.out_inter_dim, kernel_size=(1, 3, 3), stride=1,
                              padding=(0, 1, 1), bias=False)
    def forward(self, x):
        x_1 = self.conv_1x1x1_1(x)
        x1 = x_1[:, 0:self.inter_dim, ...]
        x2 = x_1[:, self.inter_dim:self.inter_dim * 2, ...]
        x3 = x_1[:, self.inter_dim * 2:self.inter_dim * 3, ...]
        x4 = x_1[:, self.inter_dim * 3:self.inter_dim * 4, ...]

        x1 = self.conv_331_1(x1)
        x2 = self.conv_331_2(x1 + x2)
        x2 = self.bn_layer_2(x2)
        x3 = self.conv_331_3( x2 + x3)
        x3 = self.bn_layer_3(x3)
        x4 = self.pool(x3 + x4)
        x4 = self.conv_331_4(x4)
        ...
        x_1 = torch.cat([x1, x2, x3, x4], dim=1)  # -> [1, 48, 32, 32, 32]
        x_1 = self.conv_1x1x1_2(x_1)
        if self.in_dim > self.out_dim:
            x = self.conv_1x1x1_3(x)
        x_1 = self.conv_133(x_1)
        return x_1 + x
